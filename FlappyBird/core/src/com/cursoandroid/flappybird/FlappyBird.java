...
public class FlappyBird extends ApplicationAdapter {
	...
	private BitmapFont fonte; // Conseguimos desenhar um texto
	...
	//Câmera (ajustar tamanho proprorcionalmente conforme tamamanho de resolução de tela)
	...
	public void render () {
		...
		//Limpar frames anteriores (limpar memória)
		...
		//Gdx.app.log("Variacao", "Varicao: " + Gdx.graphics.getDeltaTime());

		if (variacao > 2) {
			variacao = 0;
		}

		if (estadoJogo == 0) { //Se o jogo não estiver iniciado
			...
			if (estadoJogo == 1) {
				...
				if (Gdx.input.justTouched()) { // Se a tela for tocada
					...
					velocidadeQueda = -15; //Subir o passaro
				}
		...
		if (Intersector.overlaps(passaroCirculo, retanguloCanoBaixo) || Intersector.overlaps(passaroCirculo, retanguloCanoTopo)
				|| posicaoInicialVertical <= 0 || posicaoInicialVertical >= alturaDispositivo) { //Se o passoro caiu ou bateu em cima
